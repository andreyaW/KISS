# My original implementation of the Markov Chain class (less efficient)
# import numpy as np
...
# Chat GPT optimized implementation of the Markov Chain class (more efficient)
import numpy as np
import networkx as nx
import matplotlib.pyplot as plt
import numpy as np
...
class MarkovChain:
    def __init__(self, states: dict, transition_matrix: np.array) -> None:
        ...
        self.history = [self.state]                # history as a list (compatible with other modules)

    def simulate(self, number_of_steps: int = 1) -> None:
        """ Simulate the Markov Chain efficiently while keeping history correct """
        states = np.arange(len(self.states), dtype=int)
        cum_probs = np.cumsum(self.transitionMatrix, axis=1)
        current_state = self.state

        # Generate all random numbers at once
        r = np.random.rand(number_of_steps)
        for i in range(number_of_steps):
            current_state = np.searchsorted(cum_probs[current_state], r[i])
    ...
    def get_failure_time(self):
        ...
        idx = np.argmax(hist == failure_state)
        return idx if hist[idx] == failure_state else None
